// ==============================
// Catálogo de productos con CRUD, localStorage y modo oscuro
...
function aplicarTema(esOscuro) {
  if (esOscuro) {
    ...
    $('#toggle-darkmode .ui-btn-text').text('☀️');
    $('#toggle-darkmode .ui-icon').removeClass('ui-icon-bullets').addClass('ui-icon-edit');
  } else {
    $('body').removeClass('dark-mode');
    $('#toggle-darkmode .ui-btn-text').text('🌙');
    $('#toggle-darkmode .ui-icon').removeClass('ui-icon-edit').addClass('ui-icon-bullets');
  }
...
$(document).on('pagebeforeshow', '#catalogo', function() {
  var darkModeGuardado = localStorage.getItem('darkMode') === 'true';
  aplicarTema(darkModeGuardado);
});

// Evento para cambiar tema (delegación para asegurar que funciona con jQuery Mobile)
...
function cargarProductos() {
  ...
  if (stored) {
    ...
    $.getJSON("data/productos.json", function(productos) {
      ...
      console.log("Error cargando productos.json");
    });
  }
}
...
// ==============================
// INICIALIZACIÓN PÁGINA CATÁLOGO
...
// ==============================
// LIMPIAR FORMULARIO AL ABRIR (si es nuevo)
